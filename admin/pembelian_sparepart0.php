...
function stockDelta(PDO $pdo, int $sparepartId, float $delta): void
{
    $stmt = $pdo->prepare("
        ...
        LIMIT 1
    ");
    $stmt->execute([$delta, $sparepartId]);
}
    ...
    try {
        ...
        */
        if ($action === 'create') {
            ...
            $stmt = $pdo->prepare("
                SELECT id, kode, nama, stok, harga_beli
                FROM sparepart
                WHERE id = ?
                LIMIT 1
            ");
                            ...
                            >
                                ...
                                <td>
                                    <button
                                        ...
                                        onclick="openDetailModal(<?php echo (int)$row['id']; ?>)"
                                    >
                                        Detail
                                    </button>
                                </td>
                        ...
                        <select name="sparepart_id" id="createSparepart" required>
                            ...
                            <?php foreach ($spareparts as $sparepart): ?>
                                <option
                                    ...
                                    data-harga="<?php echo h($sparepart['harga_beli']); ?>"
...
(function () {
    ...
    function bindPriceSelect(selectId, priceId, infoId) {
        ...
        select.addEventListener('change', function () {
            ...
            var harga = Number(option.dataset.harga || 0);
            var stok = Number(option.dataset.stok || 0);
            var satuan = option.dataset.satuan || '';

            if (price) price.value = harga;

            if (info) {
                info.textContent =
                    'Harga master: Rp ' + formatNumber(harga) +
                    ' | Stok saat ini: ' + formatNumber(stok) +
                    (satuan ? ' ' + satuan : '');
            }
    ...
    bindPriceSelect('createSparepart', 'createHarga', 'createSparepartInfo');
    bindPriceSelect('addDetailSparepart', 'addDetailHarga', 'addDetailSparepartInfo');

    var editSelect = document.getElementById('editDetailSparepart');
    if (editSelect) {
        editSelect.addEventListener('change', function () {
            var option = editSelect.options[editSelect.selectedIndex];
            if (option && option.value) {
                document.getElementById('editDetailHarga').value =
