    ...
    try {
        ...
        elseif ($action === 'edit_sparepart') {
            ...
            foreach ($ids as $sid) {
                ...
                $lock->execute([$sid]);
                $master = $lock->fetch();
                if (!$master) throw new RuntimeException('Master sparepart tidak ditemukan.');
                $masters[$sid] = $master;
            }
...
.detail-table{width:100%;border-collapse:collapse;min-width:700px}.detail-table.wide-history{min-width:1250px}
...
.price-allocation{margin-top:14px;padding-top:14px;border-top:1px solid #dfe5ed}.price-allocation-title{text-align:center;color:#536984;font-size:10px;margin-bottom:10px}.price-allocation-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.price-allocation-grid>div{text-align:center;padding:8px 6px;background:#fafbfd;border:1px solid #e4e9f0;border-radius:3px}.price-allocation-grid span{display:block;color:#8190a5;font-size:9px;margin-bottom:4px}.price-allocation-grid strong{display:block;color:#13294b;font-size:11px}
...
@media(max-width:800px){.detail-grid,.two-col,.image-grid,.form-grid,.form-grid.three,.price-allocation-grid{grid-template-columns:1fr}.detail-title-row{flex-direction:column}.detail-field{grid-template-columns:125px 1fr}.price-grid{grid-template-columns:1fr}.price-item{border-right:0;border-bottom:1px solid #dfe5ed}.price-item:last-child{border-bottom:0}}
</style>
    ...
    <section class="detail-card">
        ...
        <div class="detail-card-body">
            <div class="price-grid">
                <div class="price-item"><span class="small">Harga Beli Unit</span><span class="big">Rp <?php echo rupiah($hargaBeliUnit); ?></span></div>
            ...
            <div class="price-allocation">
                <div class="price-allocation-title">Alokasi Biaya Pembelian per Unit<?php echo $jumlahUnitPembelian > 0 ? ' ('.$jumlahUnitPembelian.' unit dalam nota)' : ''; ?></div>
    ...
    <section class="detail-card">
        ...
        <div class="detail-table-wrap">
            <table class="detail-table wide-history">
                <thead><tr><th>No</th><th>Tanggal</th><th>No Pembelian</th><th>Supplier</th><th>Kurs</th><th>Hrg IDR</th><th>Hrg USD</th><th>Unit</th><th>Bea Cukai/Unit</th><th>Pengiriman/Unit</th><th>Biaya Lain/Unit</th><th>HPP Pembelian</th><th>Status</th></tr></thead>
                <tbody>
                <?php if (!$purchaseHistory): ?>
                    <tr><td colspan="13" class="detail-empty">Belum ada riwayat pembelian.</td></tr>
                <?php else: foreach ($purchaseHistory as $n=>$ph): ?>
                    <tr>
                        <td><?php echo $n+1; ?></td>
                        ...
                        <td class="number"><?php echo strtoupper((string)$ph['status']) === 'SELESAI' ? 'Rp '.rupiah($ph['hpp_pembelian_unit']) : '-'; ?></td>
