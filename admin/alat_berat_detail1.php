...
function dateId($value): string {
    ...
    return $time ? date('d-M-Y', $time) : h($value);
}
...
function redirectSelf(int $id): never {
...
function tableColumns(PDO $pdo, string $table): array {
    ...
    $rows = $pdo->query("SHOW COLUMNS FROM `{$safe}`")->fetchAll();
...
/*
 * Tabel image dibuat otomatis karena sebelumnya belum termasuk
 * 15 tabel utama. Jika user database tidak mempunyai privilege CREATE,
 * halaman tetap berjalan; hanya fitur image yang tidak tersedia.
 */
try {
    $pdo->exec("
        CREATE TABLE IF NOT EXISTS alat_berat_image (
            ...
            CONSTRAINT fk_abi_alat_berat
                FOREIGN KEY (alat_berat_id)
                REFERENCES alat_berat(id)
                ON UPDATE CASCADE
                ON DELETE CASCADE,
...
/* =========================================================
   ACTIONS CRUD DALAM FILE YANG SAMA
   ========================================================= */
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($action === 'update_unit') {
            $unitId = filter_input(INPUT_POST, 'id', FILTER_VALIDATE_INT);

            if (!$unitId || $unitId !== (int)$id) {
                throw new RuntimeException('ID unit tidak valid.');
            }
        ...
        elseif ($action === 'add_sparepart') {
            ...
            $qty = cleanMoney($_POST['qty'] ?? 0);
            $harga = cleanMoney($_POST['harga'] ?? 0);
            $keterangan = trim((string)($_POST['keterangan'] ?? ''));

            if (!$sparepartId) throw new RuntimeException('Sparepart wajib dipilih.');
            if ($qty <= 0) throw new RuntimeException('Qty harus lebih dari 0.');

            $check = $pdo->prepare("SELECT id FROM sparepart WHERE id = ? LIMIT 1");
            $check->execute([$sparepartId]);
            if (!$check->fetch()) throw new RuntimeException('Sparepart tidak ditemukan.');
            ...
            $message = 'Sparepart berhasil ditambahkan ke unit.';
        }
        ...
        elseif ($action === 'edit_sparepart') {
            ...
            $stmt = $pdo->prepare("
                ...
                LIMIT 1
            ");
            $stmt->execute([$sparepartId, $qty, $harga, $keterangan, $relId, $id]);
        ...
        elseif ($action === 'add_jasa') {
            ...
            $keterangan = trim((string)($_POST['keterangan'] ?? ''));
            $qty = cleanMoney($_POST['qty'] ?? 1);
            $biaya = cleanMoney($_POST['biaya'] ?? 0);
        ...
        elseif ($action === 'add_perawatan') {
            $tanggal = trim((string)($_POST['tanggal'] ?? ''));
            $keterangan = trim((string)($_POST['keterangan'] ?? ''));
            $finishing = cleanMoney($_POST['finishing'] ?? 0);
            $sukuCadang = cleanMoney($_POST['suku_cadang'] ?? 0);
            $jasa = cleanMoney($_POST['jasa'] ?? 0);
        ...
        elseif ($action === 'upload_image') {
            if (!$imageTableReady) {
                throw new RuntimeException('Tabel image belum dapat dibuat. Pastikan user database memiliki privilege CREATE.');
            }

            if (!isset($_FILES['image']) || $_FILES['image']['error'] !== UPLOAD_ERR_OK) {
                throw new RuntimeException('File image belum dipilih atau gagal diupload.');
            }
            ...
            if (!isset($allowed[$mime])) {
                throw new RuntimeException('Format image hanya JPG/JPEG atau PNG.');
            }

            if (@getimagesize($file['tmp_name']) === false) {
                throw new RuntimeException('File yang diupload bukan image yang valid.');
            }

            $uploadDir = __DIR__ . '/../assets/uploads/alat_berat';
            if (!is_dir($uploadDir) && !mkdir($uploadDir, 0755, true)) {
                throw new RuntimeException('Folder upload tidak dapat dibuat.');
            }

            $filename = 'alat_' . $id . '_' . date('YmdHis') . '_' .
            ...
            $relativePath = 'assets/uploads/alat_berat/' . $filename;

            $stmt = $pdo->prepare("
                INSERT INTO alat_berat_image
                    (alat_berat_id, file_name, file_path)
                VALUES (?, ?, ?)
            ");
            $stmt->execute([$id, $file['name'], $relativePath]);
        ...
        elseif ($action === 'delete_image') {
            ...
            if ($image) {
                $physical = dirname(__DIR__) . '/' . ltrim($image['file_path'], '/');
        ...
        $map = [
            'add_sparepart' => 'sparepart',
            'edit_sparepart' => 'sparepart',
            'add_jasa' => 'jasa',
            'edit_jasa' => 'jasa',
            'add_perawatan' => 'perawatan',
            'edit_perawatan' => 'perawatan',
            'upload_image' => 'image'
        ];

        $openModal = $map[$_POST['action'] ?? ''] ?? '';
    }
}
...
try {
    $masterSparepart = $pdo->query("
        SELECT id, kode, nama, part_number, merk, satuan
        FROM sparepart
        ORDER BY nama ASC
    ")->fetchAll();
} catch (Throwable $e) {
...
try {
    $stmt = $pdo->prepare("
        SELECT abs.*, s.kode, s.nama, s.part_number, s.satuan
...
.detail-btn-danger{border-color:#dc3545;color:#dc3545}
...
.detail-table{width:100%;border-collapse:collapse;min-width:650px}
...
.section-action{display:inline-flex;align-items:center;gap:5px;height:29px;padding:0 11px;border:1px solid #0d6efd;border-radius:4px;background:#fff;color:#0d6efd;font-size:11px;cursor:pointer}
...
.image-grid{display:grid;grid-template-columns:240px 1fr;gap:18px}
.upload-help{font-size:11px;color:#536984;margin-bottom:7px}
.upload-line{display:flex;height:34px}
.upload-line input{min-width:0;flex:1;border:1px solid #bdcbe0;border-radius:4px 0 0 4px;font-size:10px;padding:5px}
.upload-line button{width:66px;border:0;border-radius:0 4px 4px 0;background:#0d6efd;color:#fff;font-size:10px;cursor:pointer}
...
/* modal */
.modal{position:fixed;inset:0;z-index:10000;background:rgba(15,23,42,.56);display:none;align-items:center;justify-content:center;padding:12px}
.modal.show{display:flex}.modal-dialog{width:min(680px,100%);max-height:94vh;overflow:auto;background:#fff;border-radius:5px;box-shadow:0 20px 60px rgba(0,0,0,.25)}
.modal-header{height:46px;padding:0 14px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #dfe5ed}
.modal-header strong{font-size:13px;color:#17243d}.modal-close{border:0;background:transparent;color:#7d8da4;font-size:22px;cursor:pointer}
...
@media(max-width:800px){.detail-grid,.two-col,.image-grid,.form-grid,.form-grid.three{grid-template-columns:1fr}.detail-title-row{flex-direction:column}.detail-field{grid-template-columns:125px 1fr}.image-grid{gap:12px}}
</style>

<div class="detail-page">

    <div class="detail-breadcrumb">
        <span>Alat Berat</span><span class="separator">/</span><span>Detail</span>
    </div>
    ...
    <section class="detail-card">
        <div class="detail-card-header">
            ...
            <div class="detail-actions">
                <button type="button" class="detail-btn" data-open="modalEditUnit">Edit</button>
            </div>
        </div>
    ...
    <section class="detail-card">
        <div class="detail-card-header">
            ...
            <button type="button" class="section-action" data-open="modalSparepart">+ Tambah Sparepart</button>
        </div>
        <div class="detail-table-wrap">
            <table class="detail-table">
                <thead><tr>
                    <th>No</th><th>Kode</th><th>Nama Sparepart</th><th>Qty</th>
                    <th class="number">Harga</th><th class="number">Total</th><th class="center">Aksi</th>
                </tr></thead>
                <tbody>
                <?php if (!$spareparts): ?>
                    <tr><td colspan="7" class="detail-empty">Belum ada sparepart pembentuk unit.</td></tr>
                <?php else: ?>
                    <?php foreach ($spareparts as $n => $sp): ?>
                        <tr>
                            <td><?php echo $n + 1; ?></td>
                            ...
                            <td>
                                <div class="row-actions">
                                    <button
                                        type="button"
                                        class="row-btn"
                                        data-open="modalSparepartEdit"
                                        data-id="<?php echo (int)$sp['id']; ?>"
                                        data-sparepart="<?php echo (int)$sp['sparepart_id']; ?>"
                                        data-qty="<?php echo h($sp['qty']); ?>"
                                        data-harga="<?php echo h($sp['harga']); ?>"
                                        data-keterangan="<?php echo h($sp['keterangan']); ?>"
                                    >Edit</button>
                                    <form method="post" onsubmit="return confirm('Hapus sparepart dari unit ini?');">
                                        <input type="hidden" name="csrf_token" value="<?php echo h($csrf); ?>">
                                        <input type="hidden" name="action" value="delete_sparepart">
                                        <input type="hidden" name="rel_id" value="<?php echo (int)$sp['id']; ?>">
    ...
    <section class="detail-card">
        <div class="detail-card-header">
            <span>Jasa Pembentuk Unit</span>
            <button type="button" class="section-action" data-open="modalJasa">+ Tambah Jasa</button>
        </div>
        <div class="detail-table-wrap">
            <table class="detail-table">
                <thead><tr>
                    <th>No</th><th>Jasa</th><th>Keterangan</th><th>Qty</th>
                    <th class="number">Biaya</th><th class="number">Total</th><th class="center">Aksi</th>
                </tr></thead>
                <tbody>
                <?php if (!$jasas): ?>
                    <tr><td colspan="7" class="detail-empty">Belum ada data jasa.</td></tr>
                <?php else: ?>
                    <?php foreach ($jasas as $n => $j): ?>
                        <tr>
                            <td><?php echo $n + 1; ?></td>
                            ...
                            <td>
                                <div class="row-actions">
                                    <button
                                        type="button"
                                        class="row-btn"
                                        data-open="modalJasaEdit"
                                        data-id="<?php echo (int)$j['id']; ?>"
                                        data-jenis="<?php echo h($j['jenis_jasa']); ?>"
                                        data-keterangan="<?php echo h($j['keterangan']); ?>"
                                        data-qty="<?php echo h($j['qty']); ?>"
                                        data-biaya="<?php echo h($j['biaya']); ?>"
                                        data-tanggal="<?php echo h($j['tanggal']); ?>"
                                    >Edit</button>
                                    <form method="post" onsubmit="return confirm('Hapus data jasa ini?');">
                                        <input type="hidden" name="csrf_token" value="<?php echo h($csrf); ?>">
                                        <input type="hidden" name="action" value="delete_jasa">
                                        <input type="hidden" name="jasa_id" value="<?php echo (int)$j['id']; ?>">
                                        <button class="row-btn danger" type="submit">Hapus</button>
                                    </form>
                                </div>
                            </td>
    ...
    <section class="detail-card">
        <div class="detail-card-header">
            ...
            <button type="button" class="section-action" data-open="modalPerawatan">+ Perawatan Baru</button>
        </div>
        <div class="detail-table-wrap">
            <table class="detail-table">
                <thead><tr>
                    <th>Tanggal</th><th class="number">Finishing</th>
                    <th class="number">Suku Cadang</th><th class="number">Jasa</th>
                    <th class="number">Total</th><th class="center">Aksi</th>
                </tr></thead>
                <tbody>
                <?php if (!$perawatan): ?>
                ...
                <?php else: ?>
                    <?php foreach ($perawatan as $p): ?>
                        <tr>
                            <td><?php echo dateId($p['tanggal']); ?></td>
                            <td class="number"><?php echo rupiah($p['finishing']); ?></td>
                            <td class="number"><?php echo rupiah($p['suku_cadang']); ?></td>
                            <td class="number"><?php echo rupiah($p['jasa']); ?></td>
                            <td class="number"><?php echo rupiah($p['total']); ?></td>
                            <td>
                                <div class="row-actions">
                                    <button
                                        type="button"
                                        class="row-btn"
                                        data-open="modalPerawatanEdit"
                                        data-id="<?php echo (int)$p['id']; ?>"
                                        data-tanggal="<?php echo h($p['tanggal']); ?>"
                                        data-keterangan="<?php echo h($p['keterangan']); ?>"
                                        data-finishing="<?php echo h($p['finishing']); ?>"
                                        data-suku="<?php echo h($p['suku_cadang']); ?>"
                                        data-jasa="<?php echo h($p['jasa']); ?>"
                                    >Edit</button>
                                    <form method="post" onsubmit="return confirm('Hapus data perawatan ini?');">
                                        <input type="hidden" name="csrf_token" value="<?php echo h($csrf); ?>">
                                        <input type="hidden" name="action" value="delete_perawatan">
                                        <input type="hidden" name="perawatan_id" value="<?php echo (int)$p['id']; ?>">
                                        <button class="row-btn danger" type="submit">Hapus</button>
                                    </form>
                                </div>
                            </td>
        ...
        <div class="detail-card-body">
            <div class="image-grid">
                <div>
                    ...
                    <?php else: ?>
                        <div class="alert error" style="margin:0;">Tabel image belum tersedia.</div>
                    <?php endif; ?>
                </div>
                ...
                <div>
                    ...
                    <?php else: ?>
                        <div class="image-grid" style="grid-template-columns:repeat(auto-fill,minmax(120px,1fr));">
                            <?php foreach ($images as $img): ?>
                                <div class="image-item">
                                    <a href="../<?php echo h($img['file_path']); ?>" target="_blank">
                                        <img src="../<?php echo h($img['file_path']); ?>" alt="<?php echo h($img['file_name']); ?>">
                                    </a>
                                    <div class="image-item-footer">
                                        ...
                                        <form method="post" onsubmit="return confirm('Hapus image ini?');">
                                            ...
                                            <button type="submit" class="image-delete">Hapus</button>
                                        </form>
                                    </div>
                                </div>
                            <?php endforeach; ?>
    ...
    <div class="modal-dialog">
        ...
        <form method="post">
            ...
            <div class="modal-body">
                <?php if ($openModal === 'unit' && $error): ?><div class="alert error"><?php echo h($error); ?></div><?php endif; ?>
                <div class="form-grid three">
                    ...
                    <div class="form-field"><label>Tahun Pembuatan</label><input type="number" name="tahun_pembuatan" value="<?php echo h($unit['tahun_pembuatan']); ?>"></div>
                    <div class="form-field"><label>Kondisi</label>
                        <select name="kondisi">
                            <option value="Baru" <?php echo $unit['kondisi'] === 'Baru' ? 'selected' : ''; ?>>Baru</option>
                            <option value="Bekas" <?php echo $unit['kondisi'] === 'Bekas' ? 'selected' : ''; ?>>Bekas</option>
                        </select>
                    </div>
                    <div class="form-field"><label>Status</label>
                        <select name="status">
                            <?php foreach (['Tersedia','Siap Jual','Perbaikan','Disewakan','Terjual'] as $s): ?>
                                <option value="<?php echo h($s); ?>" <?php echo strcasecmp((string)$unit['status'],$s)===0?'selected':''; ?>><?php echo h($s); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
...
<div class="modal" id="modalSparepart">
    <div class="modal-dialog">
        <div class="modal-header"><strong>Tambah Sparepart Pembentuk Unit</strong><button type="button" class="modal-close" data-close>×</button></div>
        <form method="post">
            <input type="hidden" name="csrf_token" value="<?php echo h($csrf); ?>">
            <input type="hidden" name="action" value="add_sparepart">
            <div class="modal-body">
                <?php if ($openModal === 'sparepart' && $error): ?><div class="alert error"><?php echo h($error); ?></div><?php endif; ?>
                <div class="form-grid">
                        ...
                        <select name="sparepart_id" required>
                            ...
                            <?php foreach ($masterSparepart as $sp): ?>
                                <option value="<?php echo (int)$sp['id']; ?>">
                                    <?php echo h($sp['kode'].' - '.$sp['nama'].' ('.$sp['satuan'].')'); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-field"><label>Qty *</label><input name="qty" value="1" required></div>
                    <div class="form-field"><label>Harga</label><input name="harga" value="0"></div>
...
<div class="modal" id="modalSparepartEdit">
    <div class="modal-dialog">
        <div class="modal-header"><strong>Edit Sparepart Pembentuk Unit</strong><button type="button" class="modal-close" data-close>×</button></div>
        <form method="post">
            <input type="hidden" name="csrf_token" value="<?php echo h($csrf); ?>">
            <input type="hidden" name="action" value="edit_sparepart">
            <input type="hidden" name="rel_id" id="sp_rel_id">
            <div class="modal-body">
                <?php if ($openModal === 'sparepart' && $error): ?><div class="alert error"><?php echo h($error); ?></div><?php endif; ?>
                <div class="form-grid">
                    <div class="form-field full"><label>Sparepart *</label>
                        <select name="sparepart_id" id="sp_sparepart_id" required>
                            <?php foreach ($masterSparepart as $sp): ?>
                                <option value="<?php echo (int)$sp['id']; ?>"><?php echo h($sp['kode'].' - '.$sp['nama'].' ('.$sp['satuan'].')'); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-field"><label>Qty *</label><input name="qty" id="sp_qty" required></div>
                    <div class="form-field"><label>Harga</label><input name="harga" id="sp_harga"></div>
                    <div class="form-field full"><label>Keterangan</label><textarea name="keterangan" id="sp_keterangan"></textarea></div>
                </div>
            </div>
...
<div class="modal" id="modalJasa">
    <div class="modal-dialog">
        <div class="modal-header"><strong>Tambah Jasa Pembentuk Unit</strong><button type="button" class="modal-close" data-close>×</button></div>
        <form method="post">
            <input type="hidden" name="csrf_token" value="<?php echo h($csrf); ?>">
            <input type="hidden" name="action" value="add_jasa">
            <div class="modal-body">
                <?php if ($openModal === 'jasa' && $error): ?><div class="alert error"><?php echo h($error); ?></div><?php endif; ?>
                <div class="form-grid">
                    <div class="form-field"><label>Jenis Jasa *</label><input name="jenis_jasa" required placeholder="Contoh: Perakitan"></div>
                    <div class="form-field"><label>Tanggal</label><input type="date" name="tanggal" value="<?php echo date('Y-m-d'); ?>"></div>
...
<div class="modal" id="modalJasaEdit">
    <div class="modal-dialog">
        <div class="modal-header"><strong>Edit Jasa Pembentuk Unit</strong><button type="button" class="modal-close" data-close>×</button></div>
        <form method="post">
            <input type="hidden" name="csrf_token" value="<?php echo h($csrf); ?>">
            <input type="hidden" name="action" value="edit_jasa">
            <input type="hidden" name="jasa_id" id="jasa_id">
            <div class="modal-body">
                <?php if ($openModal === 'jasa' && $error): ?><div class="alert error"><?php echo h($error); ?></div><?php endif; ?>
                <div class="form-grid">
                    <div class="form-field"><label>Jenis Jasa *</label><input name="jenis_jasa" id="jasa_jenis" required></div>
                    <div class="form-field"><label>Tanggal</label><input type="date" name="tanggal" id="jasa_tanggal"></div>
                    ...
                    <div class="form-field full"><label>Keterangan</label><textarea name="keterangan" id="jasa_keterangan"></textarea></div>
                </div>
            </div>
            <div class="modal-footer"><button type="button" class="btn-secondary" data-close>Batal</button><button class="btn-primary">Update</button></div>
        </form>
...
<div class="modal" id="modalPerawatan">
    <div class="modal-dialog">
        <div class="modal-header"><strong>Tambah Perawatan</strong><button type="button" class="modal-close" data-close>×</button></div>
        <form method="post">
            <input type="hidden" name="csrf_token" value="<?php echo h($csrf); ?>">
            <input type="hidden" name="action" value="add_perawatan">
            <div class="modal-body">
                <?php if ($openModal === 'perawatan' && $error): ?><div class="alert error"><?php echo h($error); ?></div><?php endif; ?>
                <div class="form-grid">
                    <div class="form-field"><label>Tanggal *</label><input type="date" name="tanggal" value="<?php echo date('Y-m-d'); ?>" required></div>
                    <div class="form-field"><label>Finishing</label><input name="finishing" value="0"></div>
                    <div class="form-field"><label>Suku Cadang</label><input name="suku_cadang" value="0"></div>
                    <div class="form-field"><label>Jasa</label><input name="jasa" value="0"></div>
                    <div class="form-field full"><label>Keterangan</label><textarea name="keterangan"></textarea></div>
                </div>
            </div>
            <div class="modal-footer"><button type="button" class="btn-secondary" data-close>Batal</button><button class="btn-primary">Simpan</button></div>
...
<div class="modal" id="modalPerawatanEdit">
    <div class="modal-dialog">
        <div class="modal-header"><strong>Edit Perawatan</strong><button type="button" class="modal-close" data-close>×</button></div>
        <form method="post">
            <input type="hidden" name="csrf_token" value="<?php echo h($csrf); ?>">
            <input type="hidden" name="action" value="edit_perawatan">
            <input type="hidden" name="perawatan_id" id="perawatan_id">
            <div class="modal-body">
                <?php if ($openModal === 'perawatan' && $error): ?><div class="alert error"><?php echo h($error); ?></div><?php endif; ?>
                <div class="form-grid">
                    <div class="form-field"><label>Tanggal *</label><input type="date" name="tanggal" id="per_tanggal" required></div>
                    <div class="form-field"><label>Finishing</label><input name="finishing" id="per_finishing"></div>
                    <div class="form-field"><label>Suku Cadang</label><input name="suku_cadang" id="per_suku"></div>
                    <div class="form-field"><label>Jasa</label><input name="jasa" id="per_jasa"></div>
                    <div class="form-field full"><label>Keterangan</label><textarea name="keterangan" id="per_keterangan"></textarea></div>
                </div>
            </div>
            <div class="modal-footer"><button type="button" class="btn-secondary" data-close>Batal</button><button class="btn-primary">Update</button></div>
        </form>
...
(function(){
    ...
    document.querySelectorAll('[data-open]').forEach(function(btn){
        btn.addEventListener('click', function(){
            ...
            if(id === 'modalSparepartEdit'){
                ...
                document.getElementById('sp_sparepart_id').value = this.dataset.sparepart || '';
                document.getElementById('sp_qty').value = this.dataset.qty || '';
                document.getElementById('sp_harga').value = this.dataset.harga || '';
                document.getElementById('sp_keterangan').value = this.dataset.keterangan || '';
            }
    ...
    <?php if ($openModal === 'sparepart'): ?>
    openModal('modalSparepart');
    <?php elseif ($openModal === 'jasa'): ?>
    openModal('modalJasa');
    <?php elseif ($openModal === 'perawatan'): ?>
    openModal('modalPerawatan');
    <?php endif; ?>
})();
</script>
