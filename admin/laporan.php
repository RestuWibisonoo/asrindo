...
foreach ($monthlyRows as $row) {
    foreach ([
        'penjualan','non_penjualan','pemasukan','belanja_pembelian',
        'modal_aset','beban_operasional','pengeluaran_lainnya','pengeluaran'
    ] as $numericKey) {
...
$stmt = $pdo->prepare("
    SELECT
        ...
        COALESCE(
            GROUP_CONCAT(
                DISTINCT CONCAT(ab.kode, ' - ', ab.tipe)
    ...
    GROUP BY
        ...
        pm.referensi, pm.keterangan, p.nomor_penjualan, k.nama
...
$reportMonthOptions = array_map(
    static fn(array $row): string => $row['periode'],
    $monthlyRows
);
...
.month-selector-copy strong{display:block;color:#172b4d;font-size:13px;margin-bottom:3px}.month-selector-copy span{color:#8390a3;font-size:11px}
.month-selector-form{display:flex;align-items:flex-end;gap:8px}.month-selector-form label{display:block;color:#718096;font-size:11px;margin-bottom:5px}.month-selector-form select{min-width:170px;border:1px solid #b9c7d8;border-radius:5px;padding:8px 9px;font-size:12px;background:#fff}
.btn-month-detail,.btn-detail-month{border:1px solid #0d6efd;background:#0d6efd;color:#fff;border-radius:5px;padding:9px 13px;cursor:pointer;font-weight:600;font-size:12px}.btn-month-detail:hover{background:#0b5ed7}
.month-summary-table{min-width:1100px}.month-summary-table th:last-child,.month-summary-table td:last-child{width:75px;text-align:center}.btn-detail-month{border-color:#b8c7dc;background:#fff;color:#315a8c;padding:6px 10px;font-size:11px}.btn-detail-month:hover{background:#f3f6fa}
.month-value-positive{color:#198754;font-weight:700}.month-value-negative{color:#dc3545;font-weight:700}
.month-modal-backdrop{display:none;position:fixed;inset:0;background:rgba(15,23,42,.5);z-index:10000;padding:20px;overflow:auto;box-sizing:border-box}.month-modal-backdrop.show{display:block}
.month-modal{width:min(1120px,100%);margin:20px auto;background:#fff;border-radius:8px;box-shadow:0 18px 55px rgba(0,0,0,.2);overflow:hidden}.month-modal-header{display:flex;justify-content:space-between;align-items:center;padding:15px 18px;border-bottom:1px solid #dce3eb}.month-modal-title{font-weight:700;color:#172b4d;font-size:16px}.month-modal-title small{display:block;color:#8390a3;font-size:10px;font-weight:400;margin-top:3px}.month-modal-close{border:0;background:transparent;color:#718096;font-size:24px;cursor:pointer}
.client-report{padding:20px}.client-report-title{text-align:center;margin-bottom:18px;color:#172b4d}.client-report-title strong{display:block;font-size:16px}.client-report-title span{display:block;font-size:13px;font-weight:600;margin-top:3px}.client-report-section{margin-top:16px}.client-report-section-title{font-weight:700;color:#172b4d;font-size:12px;text-transform:uppercase;border-bottom:1px solid #cfd8e3;padding:6px 8px;background:#f7f9fb}.client-report-table{width:100%;border-collapse:collapse}.client-report-table td{border-bottom:1px solid #e6ebf0;padding:6px 8px;font-size:11px}.client-report-table .money{text-align:right;white-space:nowrap;font-weight:600}.client-report-table .indent{padding-left:28px}.client-report-table .subtotal td{background:#f7f9fb;font-weight:700}.client-report-table .grand-total td{background:#eef3f8;font-weight:700}.client-report-table .profit td{font-size:12px;font-weight:700;border-top:2px solid #cfd8e3}.client-report-empty{padding:8px 28px;color:#9aa7b8;font-size:11px;font-style:italic}.client-report-note{margin-top:14px;color:#8390a3;font-size:10px;line-height:1.5}
...
<div class="report-page">
    ...
    <div class="report-card">
        <div class="report-card-header">
            ...
            <small>Semua bulan dalam periode filter ditampilkan. Tombol Detail membuka laporan bulan sesuai format client.</small>
        </div>
        <?php if (!$monthlyRows): ?>
        ...
        <?php else: ?>
            <div class="monthly-table-wrap">
                <table class="report-table month-summary-table">
                    <thead><tr>
                        <th>Bulan</th><th class="right">Pendapatan Penjualan</th><th class="right">Pendapatan Non Penjualan</th>
                        <th class="right">Total Pendapatan</th><th class="right">Belanja Pembelian</th><th class="right">Modal Aset</th>
                        <th class="right">Beban Operasional</th><th class="right">Total Pengeluaran</th><th class="right">Arus Kas Bersih</th><th>Detail</th>
                    </tr></thead>
                    <tbody>
                    <?php foreach ($monthlyRows as $monthly): ?>
                        ...
                        <tr>
                            ...
                            <td class="money <?php echo $monthNet >= 0 ? 'month-value-positive' : 'month-value-negative'; ?>"><?php echo rupiah($monthNet); ?></td>
