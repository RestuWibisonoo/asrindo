...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'create') {
            ...
            if ($ppn < 0 || $ppn > 100) {
                redirectMessage('error', 'PPN harus berada di antara 0 sampai 100 persen.');
            }

            if ($tanggalJatuhTempo !== '' && $tanggalJatuhTempo < $tanggal) {
                redirectMessage('error', 'Tanggal jatuh tempo tidak boleh sebelum tanggal penjualan.');
            }
            ...
            */
            ...
            // Total transaksi = subtotal + PPN.
            // Nilai total ini disimpan ke kolom penjualan.total yang NOT NULL.
            $ppnNominal = $subtotal * ($ppn / 100);
            $totalDenganPpn = $subtotal + $ppnNominal;

            // Laba dihitung dari nilai sebelum PPN karena PPN bukan pendapatan.
            $laba = $subtotal - $hpp;
            ...
            /*
            | subtotal dan laba pada penjualan_detail adalah GENERATED COLUMN
            | sehingga keduanya tidak boleh dikirim dalam INSERT.
            | Database menghitung keduanya otomatis dari kolom sumber.
            */
            $stmt = $pdo->prepare("
        ...
        if ($action === 'change_status') {
            ...
            // Tambahkan kolom transaksi penjualan yang dibutuhkan jika belum ada.
            // Aman dijalankan berulang karena hanya menambah kolom yang belum tersedia.
            $columnChecks = [
                'tanggal_jatuh_tempo' => "ALTER TABLE penjualan ADD COLUMN tanggal_jatuh_tempo DATE NULL AFTER tanggal",
                'ppn' => "ALTER TABLE penjualan ADD COLUMN ppn DECIMAL(5,2) NOT NULL DEFAULT 0 AFTER total",
                'alamat_pengiriman' => "ALTER TABLE penjualan ADD COLUMN alamat_pengiriman TEXT NULL AFTER ppn"
            ];
...
|--------------------------------------------------------------------------
| DATA MASTER
| Kolom tambahan penjualan diasumsikan sudah tersedia di database:
| tanggal_jatuh_tempo, ppn, alamat_pengiriman.
|--------------------------------------------------------------------------
*/
$customers = $pdo->query("
    SELECT id, nama
...
<style>
    ...
    .stat-blue { color: #0d6efd !important; }
    .stat-green { color: #00a65a !important; }
    .stat-orange { color: #f0a000 !important; }
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="ppn" id="ppn" class="form-control"
                               min="0" max="100" step="0.01" value="11" required
                               oninput="updateSaleCalculation()">
                        <div class="unit-info">Masukkan persentase PPN (contoh: 11 untuk 11%, 0 untuk tanpa PPN).</div>
                    </div>
        ...
        tbody.innerHTML = pageRows.map(row => `
            <tr>
                ...
                <td class="action-cell">
                    <a href="penjualan_detail.php?id=${Number(row.id)}" class="btn-small">
                        Detail
                    </a>
...
function updateSaleCalculation() {
    ...
    document.getElementById('calcPpn').textContent = formatRupiah(ppnNominal);
